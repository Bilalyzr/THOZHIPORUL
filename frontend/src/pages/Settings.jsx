...
import {
  ...
  Stack, Snackbar, CircularProgress, MenuItem, Chip, InputAdornment
} from '@mui/material';
...
function Settings() {
  ...
  const handleUIChange = (field) => (e) => {
    ...
    setPrefs((prev) => ({ ...prev, ui: { ...prev.ui, [field]: val } }));
  };
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: { xs: 2, md: 4 } }}>
        Manage your security, notifications, and appearance.
      </Typography>

      <Grid container spacing={{ xs: 2, md: 4 }}>
        <Grid size={{ xs: 12, md: 8 }}>

          {/* Security status — 2FA is mandatory, not configurable */}
          <Paper elevation={2} sx={{ mb: 4, borderRadius: 4, overflow: 'hidden', border: '2px solid #2E7D32' }}>
            <Box sx={{ bgcolor: '#1F4E79', color: 'white', px: { xs: 2, sm: 3 }, py: 1.5, display: 'flex', alignItems: 'center' }}>
              <LockIcon sx={{ mr: 1 }} />
              <Typography variant="h6">Security</Typography>
            </Box>
            <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
              <Alert severity="success" icon={<LockIcon />}>
                <strong>Two-Factor Authentication is mandatory</strong> and always enabled for admin accounts.
                It was configured during your first login via Microsoft Authenticator. This setting cannot be changed.
              </Alert>
            </Box>
          </Paper>
          ...
          <Paper elevation={2} sx={{ mb: 4, borderRadius: 4, overflow: 'hidden' }}>
            ...
            <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
              ...
              <Stack spacing={2}>
                <FormControlLabel control={<Switch checked={!!prefs.notifications.email} onChange={toggleNotif('email')} />} label="Email notifications" />
                <FormControlLabel control={<Switch checked={!!prefs.notifications.portal} onChange={toggleNotif('portal')} />} label="In-app / portal notifications" />
                <FormControlLabel control={<Switch checked={!!prefs.notifications.sms} onChange={toggleNotif('sms')} />} label="SMS notifications" />
              </Stack>
            </Box>
          </Paper>
          ...
          <Paper elevation={2} sx={{ borderRadius: 4, overflow: 'hidden' }}>
            ...
            <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3}>
                <TextField select fullWidth label="Theme" value={prefs.ui.theme} onChange={handleUIChange('theme')}>
                  <MenuItem value="light">Light</MenuItem>
                  <MenuItem value="dark">Dark</MenuItem>
                </TextField>
                <TextField select fullWidth label="Language" value={prefs.ui.language} onChange={handleUIChange('language')}
                  InputProps={{ startAdornment: <InputAdornment position="start"><TranslateIcon sx={{ color: 'text.secondary' }} /></InputAdornment> }}>
          ...
          <Box sx={{ mt: 4, textAlign: 'right' }}>
            <Button variant="contained" color="primary" size="large" onClick={handleSave} sx={{ px: 6 }} disabled={saving}
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Paper elevation={2} sx={{ p: { xs: 2, sm: 3 }, borderRadius: 4 }}>
            ...
            <Divider sx={{ my: 2 }} />
            <List dense>
              <ListItem><ListItemText primary="Software Version" secondary="THOZHIRPORUL v1.2.0" /></ListItem>
              <ListItem><ListItemText primary="Environment" secondary="Production" /></ListItem>
              <ListItem><ListItemText primary="2FA" secondary="Mandatory (admin)" /></ListItem>
